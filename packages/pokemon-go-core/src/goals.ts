...
export type GoalRoutePrefix = string; // e.g. '/pokemon-go' (web) or '' (mobile)

export function buildGoalsSummary(
  entries: PogoDexEntry[],
  collection: PokemonGoCollection | null,
  routePrefix: GoalRoutePrefix = '/pokemon-go'
): GoalsSummary {
...
function buildNextGoals(
  ...
  for (const edge of input.ready.slice(0, 5)) {
    goals.push({
      ...
      href: path('/evolutions?view=ready'),
    });
  }
  ...
  for (const edge of almost) {
    goals.push({
      ...
      href: path('/evolutions?view=missing'),
    });
  }
  ...
  for (const fam of input.incomplete.slice(0, 5)) {
    ...
    goals.push({
      ...
      href: path('/evolutions?view=unevolved'),
  ...
  if (input.missingPokedex > 0) {
    goals.push({
      ...
      href: path('/missing?filter=missing'),
  ...
  for (const s of specials) {
    ...
    goals.push({
      ...
      href: path(`/missing?filter=${s.filter}`),
      count: s.count,
    });
  }
